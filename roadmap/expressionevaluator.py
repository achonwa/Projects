...
def tokenize(expression):
    # regular expressions to match nmbers and operators and parentheses
    pattern = r"(\d+\.\d+|\d+|[+\-*/()^])"
...
def to_postfix(tokens):
    ...
    for token in tokens:
        if token.isdigit() or re.match(r"\d+\.\d+", token):  #Number
            ...
            operators.append(token) #remove the '(' form the stack
        elif token == '(':
...
def validate_postfix(postfix_tokens):
    ...
    for token in postfix_tokens:
        if token.isdigit() or re.match(r"\d+\.\d+", token):
            ...
            raise ValueError(f"Inavlid token in psotfix expression: {token}")
    if operand_count != 1:

        raise ValueError("invaild postfix expression:")
